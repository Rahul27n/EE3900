...
\end{align}
where $s = j\omega$ and $w = 2\pi f$. The Laplace transform of the impulse response $y(t)$ is given by $H(s)$. To find the step response $x(t)$ we need to multiply $H(s)$ with $\dfrac{1}{s}$.

It is given by :
\begin{align}
H'(s) &= \frac{1}{s(s + \dfrac{1}{5})}\label{eq:2}
\end{align}
Hence the step response $x(t)$ using the Lemma-$\ref{tbl}$ is given by :
\begin{align}
